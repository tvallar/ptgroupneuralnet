...
model_deep_network = MLPRegressor(hidden_layer_sizes=(30), activation='relu', solver='adam', alpha=0.004, batch_size='auto', learning_rate='constant', 
    learning_rate_init=0.0025, power_t=0.5, max_iter=1000, shuffle=True, random_state=42, tol=0.0001, verbose=False, warm_start=False, 
    momentum=0.8, nesterovs_momentum=True, early_stopping=False, validation_fraction=0.1, beta_1=0.85, beta_2=0.8, epsilon=1e-08, n_iter_no_change=10)
...
normalizer = 0.002
y_out = np.zeros((len(x_b), 3)) + [p0/normalizer, p1/normalizer, p2/normalizer]
X_data = []
for i in range(len(x_b)):
    X_data.append([x_b[i], t[i], Q[i]])
X_data = np.array(X_data)
...
model_deep_network.fit(X_data, y_out)

predicted_dnn = model_deep_network.predict(X_test)
#for xt, yt in zip(predicted[:10], y_test[:10]):
    #print (xt, ' | ', yt)


print('Model Scoring Results')
print('RMSE of Parameter Predictions for DNN: ', rmse(predicted_dnn, y_test))
...
def get_graph_arrays(line_value, x_axis, model):
    ...
    for i in range(len(x_axis)):
        params_tmp = model.predict(np.array([[x_b1[i], t_1[i], Q_1[i]]]))*normalizer
        data1_tmp = (x_axis[i],x_b1[i], t_1[i], Q_1[i])
        model_curve1.append(calculate_observable(data1_tmp, params_tmp[0][0], params_tmp[0][1], params_tmp[0][2]))
...
plt.errorbar(X[line1*7:(line1+1)*7], y[line1*7:(line1+1)*7], errF[line1*7:(line1+1)*7],  None, 'bo', label='t={0} x_b={1} Q={2}'.format(t[line1*7],x_b[line1*7], Q[line1*7])) # plot the raw data
...
plt.errorbar(X[line2*7:(line2+1)*7], y[line2*7:(line2+1)*7], errF[line2*7:(line2+1)*7], None,  'go', label='t={0} x_b={1} Q={2}'.format(t[line2*7],x_b[line2*7], Q[line2*7])) # plot the raw data
...
plt.errorbar(X[line3*7:(line3+1)*7], y[line3*7:(line3+1)*7], errF[line3*7:(line3+1)*7], None,  'ro', label='t={0} x_b={1} Q={2}'.format(t[line3*7],x_b[line3*7], Q[line3*7])) # plot the raw data
