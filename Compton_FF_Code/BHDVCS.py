...
class BHDVCS(object):
    ...
    def Set4VectorProducts(self, phi) :
        ...
        self.kqp_T = ( self.QQ / ( np.sqrt( self.gg ) * np.sqrt( 1 + self.gg ) ) ) * np.sqrt ( (0.5 * self.e) / ( 1 - self.e ) ) * ( 1. + self.x * self.t / self.QQ ) * np.sin(self.theta) * np.cos( phi * self.RAD )
    ...
    def GetIUUxs(self, phi, F1, F2, ReH, ReE, ReHtilde) :
        ...
        self.AUUI = -4.0 * np.cos( phi * self.RAD ) / (self.kqp * self.kpqp) * ( ( self.QQ + self.t ) * ( 2.0 * ( self.kP + self.kpP ) * self.kk_T   + ( self.Pq * self.kqp_T ) + 2.* ( self.kpP * self.kqp ) - 2.* ( self.kP * self.kpqp ) ) +
                                                        ( self.QQ - self.t + 4.* self.kd ) * self.Pqp * ( self.kkp_T + self.kqp_T - 2.* self.kkp ) )
        self.BUUI = 2.0 * self.xi * np.cos( phi * self.RAD ) / ( self.kqp * self.kpqp) * ( ( self.QQ + self.t ) * ( 2.* self.kk_T * ( self.kd + self.kpd ) + self.kqp_T * ( self.qd - self.kqp - self.kpqp + 2.*self.kkp ) + 2.* self.kqp * self.kpd - 2.* self.kpqp * self.kd ) +
                                                                ( self.QQ - self.t + 4.* self.kd ) * ( ( self.kk_T - 2.* self.kkp ) * self.qpd - self.kkp * self.dd_T - 2.* self.kd_T * self.kqp ) ) / self.tau
        self.CUUI = 2.0 * np.cos( phi * self.RAD ) / ( self.kqp * self.kpqp) * ( -1. * ( self.QQ + self.t ) * ( 2.* self.kkp - self.kpqp - self.kqp ) * self.kd_T + ( self.QQ - self.t + 4.* self.kd ) * ( ( self.kqp + self.kpqp ) * self.kd_T + self.dd_T * self.kkp ) )
